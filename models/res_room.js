...
const ResRoom = {
  ...
  insertSome: (id, vals, cb) => {
    ...
    let endOfCode
    const queryString =
      "INSERT INTO res_rooms (reservation_id, room_type_id, check_in_date, check_out_date, adults,rate, confirmation_code, comments) VALUES (?,?,?,?,?,?,(CONCAT(CURDATE(), ?)),?);"
  ...
  updateCheckOut: (id, cb) => {
    const queryString =
      'UPDATE res_rooms SET checked_out=1, active=0 WHERE res_room_id=?;'
    connection.execute(queryString, [id], (err, result) => {
      if (err) throw err
      cb(result)
    })
  ...
  updateSome: (vals, cb) => {
    const queryString =
      'UPDATE res_rooms SET room_type_id=?, check_in_date=?, check_out_date=?, adults=?, comments=? WHERE res_room_id=?;'
