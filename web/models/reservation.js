...
const Reservation = {
  selectAll: cb => {
    const queryString =
      "SELECT r.reservation_id, r.customer_id, rr.active, c.first_name, c.last_name, c.email, rr.room_type_id, rr.checked_in, rr.checked_out, rr.res_room_id, DATE_FORMAT(r.created_at, '%b %d, %Y (%h:%i %p)') AS created_at, rr.check_in_date, rr.check_out_date, rr.comments, rr.room_id FROM reservations AS r INNER JOIN customers AS c ON r.customer_id=c.customer_id INNER JOIN res_rooms AS rr ON r.reservation_id=rr.reservation_id INNER JOIN room_types AS rt ON rr.room_type_id=rt.room_type_id ORDER BY r.reservation_id ASC, rr.res_room_id ASC;"
